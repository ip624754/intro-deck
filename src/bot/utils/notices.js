...
export function formatDmRequestReason(reason) {
  switch (reason) {
    case 'connect_linkedin_before_dm_request':
      return 'Connect LinkedIn before starting private-chat requests.';
    case 'cannot_message_self':
      return 'You cannot open a private-chat request to your own profile.';
    ...
    case 'target_profile_not_paid_unlock_mode':
      return 'This member accepts intro requests only. Paid Telegram-contact and private-chat requests are disabled.';
    case 'dm_thread_already_exists':
      return 'A private-chat request is already open for this profile.';
    case 'dm_thread_already_active':
      return 'An active private chat already exists with this member.';
    case 'dm_thread_blocked':
      return 'This private-chat path is blocked right now.';
    ...
    case 'dm_thread_not_owned_by_user':
      return 'This private-chat payment belongs to another account.';
    ...
    case 'dm_request_sent_via_pro':
      return 'This private-chat request is included in active Pro and is already waiting for recipient approval.';
    case 'dm_thread_not_active':
      return 'This conversation is not active yet.';
    case 'dm_thread_declined':
      return 'This private-chat request was declined. You can try again after the recipient cooldown.';
    ...
    default:
      return 'Could not open the private-chat request right now.';
  }
}
...
export function formatDmDecisionReason(reason) {
  switch (reason) {
    case 'dm_thread_missing':
      return 'This private chat is no longer available.';
    case 'dm_thread_not_actionable_by_user':
      return 'Only the recipient can act on this private-chat request.';
    ...
    case 'dm_thread_not_ready_for_decision':
      return 'This private-chat request is not ready for review yet.';
    case 'dm_thread_reported':
      return 'This private-chat request was reported and blocked.';
    default:
      return 'Could not update the private chat right now.';
  }
}
...
export function formatContactUnlockRequestReason(reason) {
  switch (reason) {
    case 'connect_linkedin_before_contact_unlock':
      return 'Connect LinkedIn before requesting Telegram contact.';
    case 'cannot_request_direct_contact_to_self':
      return 'You cannot request Telegram contact to your own profile.';
    case 'target_profile_missing':
      return 'The target profile is no longer available.';
    case 'target_profile_not_public':
      return 'This profile is not publicly listed right now.';
    case 'target_profile_not_paid_unlock_mode':
      return 'This profile does not accept paid Telegram-contact requests right now.';
    ...
    case 'contact_unlock_request_already_exists':
      return 'A Telegram contact request is already active for this profile.';
    case 'contact_unlock_already_revealed':
      return 'Telegram contact is already revealed for this profile.';
    ...
    case 'contact_unlock_request_not_owned_by_user':
      return 'This Telegram contact payment belongs to another account.';
    case 'contact_payment_charge_missing':
      return 'Telegram did not provide a payment charge reference. Contact support before retrying.';
    case 'contact_unlock_covered_by_pro':
      return 'This Telegram contact request used your Pro fair-use allowance and is waiting for approval.';
    case 'contact_request_cooldown_active':
      return 'This member recently declined a contact request. Try again after the shared recipient cooldown.';
    case 'contact_path_blocked':
      return 'This contact pair is blocked. No new Telegram-contact request can be opened.';
    case 'pro_outreach_daily_limit_reached':
      return 'Your Pro fair-use allowance for the rolling 24-hour window is used. Try again later or pay per request.';
    case 'contact_contract_requires_migration':
      return 'Contact requests are temporarily unavailable until the contact-contract migration is applied.';
    ...
    case 'payment_amount_mismatch':
      return 'This payment amount does not match the request. Do not retry; contact support.';
    default:
      return 'Could not open the Telegram contact request right now.';
  }
}
...
export function formatContactUnlockDecisionReason(reason) {
  switch (reason) {
    case 'contact_unlock_request_missing':
      return 'This Telegram contact request is no longer available.';
    case 'contact_unlock_request_not_actionable_by_user':
      return 'Only the recipient can approve or decline this Telegram contact request.';
    ...
    case 'target_profile_not_paid_unlock_mode':
      return 'This profile no longer accepts paid Telegram-contact requests.';
    case 'target_profile_no_hidden_telegram_username':
      return 'No hidden Telegram username is available to reveal right now.';
    default:
      return 'Could not update the Telegram contact request right now.';
  }
}
